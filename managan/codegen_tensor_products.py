...
from codegen_tensops_cuda import (set_kern_attributes,
  ...
  cow_o0_fwd_cuda, cow_o0_bwl_cuda, cow_o0_bwr_cuda,
  cow_o1_fwd_cuda, cow_o1_bwl_cuda, cow_o1_bwr_cuda,
)
...
class Cow0(torch.autograd.Function):
  @staticmethod
  def forward(ctx, l_0, l_1, r_0, r_1):
    ctx.save_for_backward(l_0, l_1, r_0, r_1)
    y_000, y_110 = cow_o0_fwd_cuda(l_0, l_1, r_0, r_1)
    return y_000, y_110
  @staticmethod
  def backward(ctx, dy_000, dy_110):
    l_0, l_1, r_0, r_1 = ctx.saved_tensors
    dl_0, dl_1 = cow_o0_bwl_cuda(r_0, r_1, dy_000, dy_110)
    dr_0, dr_1 = cow_o0_bwr_cuda(l_0, l_1, dy_000, dy_110)
    return dl_0, dl_1, dr_0, dr_1

class Cow1(torch.autograd.Function):
  @staticmethod
  def forward(ctx, l_0, l_1, r_0, r_1):
    ctx.save_for_backward(l_0, l_1, r_0, r_1)
    y_011, y_101, y_111 = cow_o1_fwd_cuda(l_0, l_1, r_0, r_1)
    return y_011, y_101, y_111
  @staticmethod
  def backward(ctx, dy_011, dy_101, dy_111):
    l_0, l_1, r_0, r_1 = ctx.saved_tensors
    dl_0, dl_1 = cow_o1_bwl_cuda(r_0, r_1, dy_011, dy_101, dy_111)
    dr_0, dr_1 = cow_o1_bwr_cuda(l_0, l_1, dy_011, dy_101, dy_111)
    return dl_0, dl_1, dr_0, dr_1
...
if __name__ == "__main__":
  ...
  print("\nTEST: COW")
  def cow0_refimpl(l_0, l_1, r_0, r_1):
    y_000 = torch.einsum("bc, bc -> bc", l_0, r_0)
    y_110 = torch.einsum("bcx, bcx -> bc", l_1, r_1)
    return y_000, y_110

  def cow1_refimpl(l_0, l_1, r_0, r_1):
    y_011 = torch.einsum("bc, bcx -> bcx", l_0, r_1)
    y_101 = torch.einsum("bcx, bc -> bcx", l_1, r_0)
    y_111 = torch.einsum("bcx, bcy, xyz -> bcz", l_1, r_1, lc_ijk)
    return y_011, y_101, y_111
  ...
  r_0 = torch.randn(batch, chan, device="cuda", requires_grad=True)
  r_1 = torch.randn(batch, chan, 3, device="cuda", requires_grad=True)
  params = [l_0, l_1, r_0, r_1]
  param_names = ["l_0", "l_1", "r_0", "r_1"]
  ...
  # Test Cow0
  y_refimpl = cow0_refimpl(l_0, l_1, r_0, r_1)
  grad_y = [torch.randn_like(y_p) for y_p in y_refimpl]
  loss_refimpl = sum([(y_p*grad_y_p).mean() for y_p, grad_y_p in zip(y_refimpl, grad_y)])
  loss_refimpl.backward()
  grads_refimpl = [param.grad for param in params]
  ...
  zero_grad_cow()
  y_ckernel = Cow0.apply(l_0, l_1, r_0, r_1)
  loss_ckernel = sum([(y_p*grad_y_p).mean() for y_p, grad_y_p in zip(y_ckernel, grad_y)])
  loss_ckernel.backward()
  grads_ckernel = [param.grad for param in params]

  for i, output_name in enumerate("y_000, y_110".split(", ")):
    print(f"{output_name} avg_relative_diff: {avg_relative_diff(y_refimpl[i], y_ckernel[i])}")
  for param_name, grad_ref, grad in zip(param_names, grads_refimpl, grads_ckernel):
    print(f"d{param_name} avg_relative_diff: {avg_relative_diff(grad_ref, grad)}")
  ...
  # Test Cow1
  y_refimpl = cow1_refimpl(l_0, l_1, r_0, r_1)
  grad_y = [torch.randn_like(y_p) for y_p in y_refimpl]
  loss_refimpl = sum([(y_p*grad_y_p).mean() for y_p, grad_y_p in zip(y_refimpl, grad_y)])
  loss_refimpl.backward()
  grads_refimpl = [param.grad for param in params]
  ...
  zero_grad_cow()
  y_ckernel = Cow1.apply(l_0, l_1, r_0, r_1)
  loss_ckernel = sum([(y_p*grad_y_p).mean() for y_p, grad_y_p in zip(y_ckernel, grad_y)])
  loss_ckernel.backward()
  grads_ckernel = [param.grad for param in params]

  for i, output_name in enumerate("y_011, y_101, y_111".split(", ")):
    print(f"{output_name} avg_relative_diff: {avg_relative_diff(y_refimpl[i], y_ckernel[i])}")
  for param_name, grad_ref, grad in zip(param_names, grads_refimpl, grads_ckernel):
    print(f"d{param_name} avg_relative_diff: {avg_relative_diff(grad_ref, grad)}")
