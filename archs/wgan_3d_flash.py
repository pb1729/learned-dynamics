...
class Block(nn.Module):
  def __init__(self, config):
    ...
    self.probe_pts_q = ProbePoints2(2, len(config["r0_list"]), adim, vdim)
    self.probe_pts_k = ProbePoints2(2, len(config["r0_list"]), adim, vdim)
  ...
  def forward(self, tup):
    ...
    probes_q = self.probe_pts_q(y_a, y_v, [pos_0, pos_1])
    probes_k = self.probe_pts_k(y_a, y_v, [pos_0, pos_1])
