...
class Block(nn.Module):
  ...
  def forward(self, tup):
    ...
    # group norm
    x_a, x_v, x_d = self.gn_a(self.lin_out_a(x_a)), self.gn_v(self.lin_out_v(x_v)), self.gn_d(self.lin_out_d(x_d))
...
class Discriminator(nn.Module):
  def __init__(self, config, randgen):
    ...
    self.lin_v = nn.Linear(self.dim_v, config["heads"], bias=False)
    self.lin_d = TensTrace(2, self.dim_d, config["heads"])
    self.taxicab_scale = nn.Parameter(torch.tensor(0.01)) # scale down for stability!
  def forward(self, pos_0, pos_1, box):
    ...
    #y = 0.1*self.taxicab_scale*taxicab(pos_1, pos_2)[:, None] # (batch, 1)
    # activation-based output, with distinct heads
    y = self.lin_a(x_a.mean(1))                                     # (batch, heads)
    y = y + self.lin_v(torch.sqrt(1e-6 + (x_v**2).sum(-1).mean(1))) # (batch, heads)
