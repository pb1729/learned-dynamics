...
class WGAN3D:
  ...
  def __init__(self, disc, gen, config):
    ...
    self.init_optim()
  def init_optim(self):
    # TODO: put in weight decay (need to separate LayerNorm.weight, though)
    self.optim_d = FriendlyAverage34(self.disc.parameters(), self.config["lr_d"], beta=self.config["beta_2"])
    self.optim_g = FriendlyAverage34(self.gen.parameters(),  self.config["lr_g"], beta=self.config["beta_2"])
    self.step_count = 0
  @staticmethod
