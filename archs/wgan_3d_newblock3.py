...
class MessageTensorProds(nn.Module):
  def __init__(self, inds_x:int, inds_φ:int, inds_B:int, dim_x:int, dim_φ:int, dim_B:int, rank:int):
    super().__init__()
    self.lin_x = TensLinear(inds_x, dim_x, rank)
    self.lin_φ = TensLinear(inds_φ, dim_φ, rank)
    self.lin_B = TensLinear(inds_B, rank, dim_B)
    self.prod = TensorProdBare(inds_φ, inds_x, inds_B)
  def forward(self, graph, x, φ_ij):
    # we perform the evaluation so as to avoid computations on edges as much as possible.
    x = self.lin_x(x)                   # (batch, nodes, rank, (3,)^inds_x)
    x_j = edges_read_dst(graph, x)      # (edges, rank, (3,)^inds_x)
    φ_ij = self.lin_φ(φ_ij)             # (edges, rank, (3,)^inds_φ)
    B_ij = self.prod(φ_ij, x_j)         # (edges, rank, (3,)^inds_B)
    B_i = edges_reduce_src(graph, B_ij) # (batch, nodes, rank, (3,)^inds_B)
    return self.lin_B(B_i)              # (batch, nodes, dim_B, (3,)^inds_B)
...
class ACEMessageEmbed(nn.Module):
  def __init__(self, r0:float, dim_a:int, dim_v:int, dim_d:int, rank:int):
    ...
    self.r0 = r0
    # φ_a_ij prods
    self.tp_000 = MessageTensorProds(0, 0, 0, dim_a, 8, dim_a, rank)
    self.tp_101 = MessageTensorProds(1, 0, 1, dim_v, 8, dim_v, rank)
    self.tp_202 = MessageTensorProds(2, 0, 2, dim_d, 8, dim_d, rank)
    # φ_v_ij prods
    self.tp_011 = MessageTensorProds(0, 1, 1, dim_a, 8, dim_v, rank)
    self.tp_110 = MessageTensorProds(1, 1, 0, dim_v, 8, dim_a, rank)
    self.tp_112 = MessageTensorProds(1, 1, 2, dim_v, 8, dim_d, rank)
    self.tp_211 = MessageTensorProds(2, 1, 1, dim_d, 8, dim_v, rank)
    # φ_d_ij prods
    self.tp_022 = MessageTensorProds(0, 2, 2, dim_a, 8, dim_d, rank)
    self.tp_121 = MessageTensorProds(1, 2, 1, dim_v, 8, dim_v, rank)
    self.tp_220 = MessageTensorProds(2, 2, 0, dim_d, 8, dim_a, rank)
    self.tp_222 = MessageTensorProds(2, 2, 2, dim_d, 8, dim_d, rank)
  def forward(self, graph, r_ij, x_a, x_v, x_d):
    ...
    φ_v_ij = rad_enc_ij[..., None]       * r_ij[..., None, :]                                  # (edges, 8, 3)
    φ_d_ij = rad_enc_ij[..., None, None] * r_ij[..., None, None, :] * r_ij[..., None, :, None] # (edges, 8, 3, 3)
    B_a_i = self.tp_000(graph, x_a, φ_a_ij) + self.tp_110(graph, x_v, φ_v_ij) + self.tp_220(graph, x_d, φ_d_ij)
    B_v_i = self.tp_011(graph, x_a, φ_v_ij) + self.tp_101(graph, x_v, φ_a_ij) + self.tp_121(graph, x_v, φ_d_ij) + self.tp_211(graph, x_d, φ_v_ij)
    B_d_i = self.tp_022(graph, x_a, φ_d_ij) + self.tp_112(graph, x_v, φ_v_ij) + self.tp_202(graph, x_d, φ_a_ij) + self.tp_222(graph, x_d, φ_d_ij)
