...
# This source file is completely unmodified from wgan_dn
# But the basic idea of the changes we want to make:
# * use an autoencoder (specifically VAE) to represent the space
# * for initial simplicity, should probably just pretrain the VAE...
# * but the pretraining should also follow the rules of the 65536 challenge
# * since in theory we could reuse data to satisfy the rules of the challenge as a whole
# * later on, we could also add the mean-predictor network. but that's too complicated for now
# * still, should also be possible to configure various possible architectures for the VAEs
...
class GANTrainer:
  def __init__(self, disc, gen, enc, dec, config):
    self.disc = disc
    self.gen  = gen
    self.config = config
    self.init_optim()
