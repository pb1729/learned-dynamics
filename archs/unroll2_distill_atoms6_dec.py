...
def diffuser_get_encoding(model, pos_0, pos_1, metadata, steps:int):
  """ Get the latent encoding for this model.
      pos_0, pos_1: (batch, atoms, 3)
      ans: pos_1_noised, epsilon
      pos_1_noised: (batch, atoms, 3)
      epsilon: (batch, atoms, steps, 3) """
  batch, atoms, must_be[3] = pos_0.shape
  device = pos_0.device
  with torch.no_grad():
    epsilon_bwd = []
    epsilon_fwd = [torch.randn_like(pos_0) for i in range(steps)]
    pos_t = pos_1
    t_list = np.linspace(1., 0., steps + 1)
    for i in range(steps):
      t = torch.tensor([t_list[i]], device=device, dtype=torch.float32)
      tinc = torch.tensor([t_list[i + 1]], device=device, dtype=torch.float32)
      sigma_t = model.sigma_t(t)[:, None, None]
      sigma_tinc = model.sigma_t(tinc)[:, None, None]
      dsigma = torch.sqrt(sigma_tinc**2 - sigma_t**2)
      pos_tinc = pos_t + dsigma*epsilon_fwd[i]
      mean_tinc = model.dn(tinc, pos_0, pos_tinc, model.box, metadata)
      epsilon_bwd.append((
          (pos_t - pos_tinc)
          - (dsigma**2/sigma_tinc**2)*(mean_tinc - pos_tinc)
        )/dsigma)
      pos_t = pos_tinc
    return pos_t, torch.stack(epsilon_bwd, dim=2)
...
class WGAN3D:
  ...
  def train_step(self, traj_state):
    ...
    loss = self.decode_step(x, traj_state.metadata)
    self.step_count += 1
    self.sched_gen.step(self.step_count, self.optim_gen)
    return loss
  def decode_step(self, x, metadata):
    L, batch, atoms, must_be[3] = x.shape
    x_0 = x[:-1].reshape((L - 1)*batch, atoms, 3)
    x_1 = x[ 1:].reshape((L - 1)*batch, atoms, 3)
    x_1_noised, epsilon = diffuser_get_encoding(self.distill_model, x_0, x_1, metadata, self.config["diffusion_steps"])
    x_1_pred = self.gen(x_0, x_1_noised, epsilon, self.box, metadata)
