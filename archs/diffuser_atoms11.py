...
from managan.flavour_layers import ResidueAtomEmbed, LinAminoToAtomSmall, LinAtomToAminoSmall
...
class DisplacementTensors(nn.Module):
  ...
  def __init__(self, r0:float, dim_a:int, dim_v:int):
    super().__init__()
    self.r0 = r0
    self.res_emb_readin = nn.Linear(dim_a, 8)
    self.mlp_radial = make_mlp(16, dim_a)
    self.readout_v = TensLinear(1, dim_a, dim_v)
  def forward(self, graph, r_ij, res_emb):
    res_emb_j = edges_read_dst(graph, self.res_emb_readin(res_emb))
