...
from managan.flavour_layers import ResiduesEncode, ResidueAtomEmbed, LinAminoToAtom, LinAtomToAmino
...
@torch.compile
def tens_prods(x_0, x_1, x_2,
    P_000, left_000, P_110, left_110, P_220, left_220,
    P_011, left_011, P_101, left_101, P_121, left_121, P_211, left_211,
    P_022, left_022, P_112, left_112, P_202, left_202, P_222, left_222):
  p_000 = torch.einsum("...l, ...r, olr -> ...o", left_000, x_0, P_000)
  p_110 = torch.einsum("...lj, ...rj, olr -> ...o", left_110, x_1, P_110)
  p_220 = torch.einsum("...ljk, ...rjk, olr -> ...o", left_220, x_2, P_220)
  p_011 = torch.einsum("...l, ...rj, olr -> ...oj", left_011, x_1, P_011)
  p_101 = torch.einsum("...lj, ...r, olr -> ...oj", left_101, x_0, P_101)
  p_121 = torch.einsum("...lj, ...rkj, olr -> ...ok", left_121, x_2, P_121)
  p_211 = torch.einsum("...ljk, ...rk, olr -> ...oj", left_211, x_1, P_211)
  p_022 = torch.einsum("...l, ...rkj, olr -> ...ojk", left_022, x_2, P_022)
  p_112 = torch.einsum("...lj, ...rk, olr -> ...ojk", left_112, x_1, P_112)
  p_202 = torch.einsum("...ljk, ...r, olr -> ...ojk", left_202, x_0, P_202)
  p_222 = torch.einsum("...ljk, ...rmk, olr -> ...ojm", left_222, x_2, P_222)
  return p_000 + p_110 + p_220, p_011 + p_101 + p_121 + p_211, p_022 + p_112 + p_202 + p_222
...
class SelfTensProds(nn.Module):
  def __init__(self, dim_a, dim_v, dim_d, dim_l):
    super().__init__()
    self.dim_l:int = dim_l
    # parameters
    add_tens_prod_parameters(self, dim_a, dim_v, dim_d, dim_l)
    # submodules:
    self.lin_a = TensLinear(0, dim_a, 3*dim_l)
    self.lin_v = TensLinear(1, dim_v, 4*dim_l)
    self.lin_d = TensLinear(2, dim_d, 4*dim_l)
  def forward(self, x_a, x_v, x_d):
    left_000, left_011, left_022           = torch.split(self.lin_a(x_a), self.dim_l, dim=-1)
    left_110, left_101, left_121, left_112 = torch.split(self.lin_v(x_v), self.dim_l, dim=-2)
    left_220, left_211, left_202, left_222 = torch.split(self.lin_d(x_d), self.dim_l, dim=-3)
    return tens_prods(
        x_a, x_v, x_d,
        self.P_000, left_000, self.P_110, left_110, self.P_220, left_220,
        self.P_011, left_011, self.P_101, left_101, self.P_121, left_121, self.P_211, left_211,
        self.P_022, left_022, self.P_112, left_112, self.P_202, left_202, self.P_222, left_222)
...
class LocalMLPAndGroupNorm(nn.Module):
...
class DisplacementTensors(nn.Module):
  ...
  def forward(self, graph, r_ij):
    rad_enc_ij = self.mlp_radial(radial_encode_edge(r_ij, 8, self.r0))
...
class Messages(nn.Module):
  ...
  def forward(self, graph, r_ij, x_a, x_v, x_d):
    ...
    ψ_a_ij, ψ_v_ij, ψ_d_ij = tens_prods(
        x_a_j, x_v_j, x_d_j,
        self.P_000, φ_a_ij, self.P_110, φ_v_ij, self.P_220, φ_d_ij,
        self.P_011, φ_a_ij, self.P_101, φ_v_ij, self.P_121, φ_v_ij, self.P_211, φ_d_ij,
        self.P_022, φ_a_ij, self.P_112, φ_v_ij, self.P_202, φ_d_ij, self.P_222, φ_d_ij)
...
class SubBlock(nn.Module):
  def __init__(self, config):
    ...
    self.prods = SelfTensProds(dim_a, dim_v, dim_d, config["dim_l"])
    self.mlps_out = LocalMLPAndGroupNorm(config)
...
class AminoSubBlock(nn.Module):
  def __init__(self, config):
    ...
    dim_a, dim_v, dim_d = config["dim_a"], config["dim_v"], config["dim_d"]
    # submodules:
    self.encode_0 = ResiduesEncode(dim_v)
    self.encode_1 = ResiduesEncode(dim_v)
    ...
    # TODO: should we include this guy?
    #self.prods = SelfTensProds(dim_a, dim_v, dim_d, config["dim_l"])
  ...
  def forward(self, tup):
    ...
    pos_1, graph_1, r_ij_1 = pos_1_tup
    # readin
    x_a, x_v, x_d = self.readin_a(x_a, metadata), self.readin_v(x_v, metadata), self.readin_d(x_d, metadata)
    # embed
    pos_0_Cα, Δ0_x_v = self.encode_0(pos_0, metadata)
    pos_1_Cα, Δ1_x_v = self.encode_1(pos_1, metadata)
    x_v = x_v + Δ0_x_v + Δ1_x_v + self.pos_embed(pos_0_Cα, pos_1_Cα) + self.ndisp_embed(pos_0_Cα, pos_1_Cα)
    # process
    x_a, x_v, x_d = self.convs(x_a, x_v, x_d)
    # readout
    return self.readout_a(x_a, metadata), self.readout_v(x_v, metadata), self.readout_d(x_d, metadata)
...
class Block(nn.Module):
  ...
  def __init__(self, config:Config, pos_1_mutable=False):
    ...
    self.messages_0 = Messages(config["r_cut"], dim_a, dim_v, dim_d)
    self.messages_1 = Messages(config["r_cut"], dim_a, dim_v, dim_d)
  ...
  def forward(self, tup):
    ...
    x_d = x_d + Δ0_x_d + Δ1_x_d
    # amino conv subblock
    Δx_a, Δx_v, Δx_d = self.amino_sub_block((t, contexttup, (x_a, x_v, x_d), pos_0_tup, pos_1_tup))
    x_a = x_a + Δx_a
    x_v = x_v + Δx_v
    x_d = x_d + Δx_d
    # update pos_1
    ...
    return t, contexttup, (x_a, x_v, x_d), pos_0_tup, pos_1_tup
...
class Denoiser(nn.Module):
  ...
  def forward(self, t, pos_0, pos_1, box, metadata):
    ...
    tup = t, contexttup, xtup, pos_0_tup, pos_1_tup
    tup = self.blocks(tup)
    t, contexttup, xtup, pos_0_tup, pos_1_tup = tup
    return pos_1_tup[0]
