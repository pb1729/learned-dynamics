...
class SymmetricDenoiser(nn.Module):
  def __init__(self, config):
    super().__init__()
    self.directed_dn = Denoiser(config)
  def forward(self, t0, t1, pos_0, pos_1, box, metadata):
    pos_0_dn_0_to_1, pos_1_dn_0_to_1 = self.directed_dn(t0, t1, pos_0, pos_1, box, metadata)
    pos_1_dn_1_to_0, pos_0_dn_1_to_0 = self.directed_dn(t1, t0, pos_1, pos_0, box, metadata)
    score_pos_0 = pos_0 - (pos_0_dn_0_to_1 + pos_0_dn_1_to_0)/2
    score_pos_1 = pos_1 - (pos_1_dn_0_to_1 + pos_1_dn_1_to_0)/2
    return score_pos_0, score_pos_1
...
class DiffusionDenoiser:
  ...
  def __init__(self, config):
    ...
    self.config = config
    self.dn = SymmetricDenoiser(config).to(config.device)
  ...
  def _diffuser_step(self, x, metadata):
    ...
    noise_0_pred, noise_1_pred = self.dn(t0, t1, x_0_noised, x_1_noised, self.box, metadata)
    loss = (((noise_0_pred - noise_0)/(0.4 + sigma_0))**2).mean() + (((noise_1_pred - noise_1)/(0.4 + sigma_1))**2).mean()
  ...
  def generate(self, x_0:torch.Tensor, metadata, steps=12):
    *leading_dims, atoms, must_be[3] = x_0.shape
    ...
    for i in range(steps):
      ...
      _, noise_1_pred = self.dn(torch.ones_like(t), t, x_0, x_1_noised, self.box, metadata)
      x_1_noised -= ((dsigma/sigma_t)**2)*noise_1_pred
