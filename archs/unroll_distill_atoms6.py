...
class Generator(nn.Module):
  def __init__(self, config):
    ...
    self.dim_a, self.dim_v, self.dim_d = config["dim_a"], config["dim_v"], config["dim_d"]
    # submodules
    self.readin_z = TensLinear(1, config["diffusion_steps"] + 1, self.dim_v)
    self.blocks = nn.Sequential(*[
...
def diffuser_generate_with_epsilon(model, pos_0, metadata, steps:int):
  """ pos_0: (batch, atoms, 3)
      ans: pos_1, epsilon     where
      pos_1: (batch, atoms, 3)
      epsilon: (batch, atoms, steps, 3) """
  batch, atoms, must_be[3] = pos_0.shape
  device = pos_0.device
  with torch.no_grad():
    epsilon = [torch.randn_like(pos_0)]
    pos_1 = pos_0 + model.sigma_t(torch.zeros(1, device=device))[:, None, None]*epsilon[-1]
  ...
  return pos_1, torch.stack(epsilon, dim=2)
...
class WGAN3D:
  ...
  def mimic_step(self, x, metadata):
    ...
    x_1, epsilon = diffuser_generate_with_epsilon(self.distill_model, x_0, metadata, self.config["diffusion_steps"])
    x_1_pred = self.gen(x_0, epsilon, self.box, metadata)
  ...
  def generate(self, x_0, metadata):
    *leading_dims, atoms, must_be[3] = x_0.shape
    ...
    epsilon = self.randgen.randn(1, (batch, atoms, self.config["diffusion_steps"] + 1))
