...
class AminoSubBlock(nn.Module):
  ...
  def forward(self, tup):
    ...
    # embed
    pos_0_amino, Δ0_x_v = self.encode_0(pos_0, metadata)
    pos_1_amino, Δ1_x_v = self.encode_1(pos_1, metadata)
    x_v = x_v + Δ0_x_v + Δ1_x_v + self.pos_embed(pos_0_amino, pos_1_amino) + self.ndisp_embed(pos_0_amino, pos_1_amino)
    # 1d conv along chain
    x_a, x_v, x_d = self.convs(x_a, x_v, x_d)
    # local computations
    x_a, x_v, x_d = self.prods(x_a, x_v, x_d)
    # time-embed, MLP, groupnorm
    x_a, x_v, x_d = self.mlps_out(x_a, x_v, x_d, t)
    # readout
