  ...
  def get_elbo_loss(self):
    ...
    for (inds, chan), (mu, trans) in zip(self.calls, self.z):
      if inds == 0:
        ...
        ans += 0.5*(torch.einsum("...ii->...", cov) + (mu*mu).sum(-1) - torch.log(torch.det(trans)**2 + 1e-4))
      else:
...
class Block(nn.Module):
  ...
  def __init__(self, config:Config, kind:int, randgen:Union[None, TensorRandGen, GaussELBORandgen]):
    ...
    if kind == self.K_DISC:
      self.t_embed = TimeEmbedding(dim_a//2, dim_a)
  ...
  def forward(self, tup):
    ...
    if self.kind == self.K_DISC:
      t, E = head
      t_embed = self.t_embed(t)[:, None]
    Δx_a, Δx_v, Δx_d = self.sub_block((
      contexttup, (
        x_a + self.res_embed(metadata) + t_embed,
    ...
    if self.kind == self.K_DISC:
      E = E + self.x_to_E(x_a)
      head = t, E
    return head, contexttup, (x_a, x_v, x_d), ytup, pos_0_tup, pos_1_tup
...
class Discriminator(nn.Module):
  ...
  def forward(self, t, pos_0, pos_1, box, metadata):
    tup = initialize_tup(self, (t, 0.), pos_0, pos_1, box, metadata)
    tup = self.blocks(tup)
    head, contexttup, xtup, ytup, pos_0_tup, pos_1_tup = tup
    t, E = head
