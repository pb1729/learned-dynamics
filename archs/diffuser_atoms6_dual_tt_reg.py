...
class TimeEmbedding(nn.Module):
  def __init__(self, hdim, outdim):
    ...
    self.hdim = hdim
    self.lin1 = nn.Linear(4*self.hdim, outdim)
  def raw_t_embed(self, t0, t1):
    """ t0, t1: (batch) """
    ang_freqs = torch.exp(-torch.arange(self.hdim, device=t0.device)/(self.hdim - 1))
    phases_0 = t0[:, None] * ang_freqs[None, :]
    phases_1 = t1[:, None] * ang_freqs[None, :]
    return torch.cat([
      torch.sin(phases_0),
      torch.cos(phases_0),
      torch.sin(phases_1),
  ...
  def forward(self, t0, t1):
    """ t0, t1: (batch)
        ans: (batch, outdim) """
    return self.lin1(self.raw_t_embed(t0, t1))
...
class TimeLinearModulation(nn.Module):
  ...
  def forward(self, x, t):
    ...
    t0, t1 = t
    x = self.lin_i(x)
    x = x * self.embed(t0, t1)[:, None, :][(...,) + (None,)*self.inds]
    return self.lin_o(x)
...
class Block(nn.Module):
  ...
  def forward(self, tup):
    ...
    x_d = x_d + Δx_d
    # update positions
    pos_0 = pos_0 + self.lin_push_pos_0(x_v).squeeze(-2)
    pos_0_tup = pos_0, None, None # invalidate previous graph and r_ij
...
class Denoiser(nn.Module):
  ...
  def forward(self, t0, t1, pos_0, pos_1, box, metadata):
    device = t0.device
    batch, = t0.shape
    must_be[batch], = t1.shape
    must_be[batch], atoms,          must_be[3] = pos_0.shape
    ...
    tup = (t0, t1), contexttup, xtup, ytup, pos_0_tup, pos_1_tup
    tup = self.blocks(tup)
    t, contexttup, xtup, ytup, pos_0_tup, pos_1_tup = tup
    x_a, x_v, x_d = xtup
    return pos_0_tup[0], pos_1_tup[0] # return positions
...
class DiffusionDenoiser:
  ...
  def __init__(self, config):
    ...
    self.init_optim()
  def init_optim(self):
    self.scheduler = ErrorTrackingScheduler(get_lr_fn_warmup_and_decay(self.config["lr"], 0.1, 256, (1. - self.config["lr_fac"])/1024))
    betas = (self.config["beta_1"], self.config["beta_2"])
    self.optim = torch.optim.AdamW(self.dn.parameters(),
      self.scheduler.lr(), betas, weight_decay=self.config["weight_decay"])
    self.step_count = 0
  @staticmethod
  ...
  def _diffuser_step(self, x, metadata):
    ...
    t0 = torch.rand((L - 1)*batch, device=x.device)
    t1 = torch.rand((L - 1)*batch, device=x.device)
    epsilon_0 = torch.randn_like(x_0)
    epsilon_1 = torch.randn_like(x_1)
    sigma_0 = self.sigma_t(t0)[:, None, None]
    sigma_1 = self.sigma_t(t1)[:, None, None]
    noise_0 = sigma_0*epsilon_0
    noise_1 = sigma_1*epsilon_1
    x_0_noised = x_0 + noise_0
    x_1_noised = x_1 + noise_1
    x_0_pred, x_1_pred = self.dn(t0, t1, x_0_noised, x_1_noised, self.box, metadata)
    x_0_pred_flip, x_1_pred_flip = self.dn(t1, t0, x_1_noised, x_0_noised, self.box, metadata)
    detailed_balance_reg_loss = (((x_0_pred - x_1_pred_flip)/(0.4 + sigma_0))**2).mean() + (((x_1_pred - x_0_pred_flip)/(0.4 + sigma_1))**2).mean()
    denoise_loss = (((x_0_pred - x_0)/(0.4 + sigma_0))**2).mean() + (((x_1_pred - x_1)/(0.4 + sigma_1))**2).mean()
    loss = denoise_loss + self.config["lambda"]*detailed_balance_reg_loss
    self.optim.zero_grad()
    loss.backward()
    self.optim.step()
    return loss.item(), denoise_loss.item(), detailed_balance_reg_loss.item()
  def generate(self, x_0:torch.Tensor, metadata, steps=12):
    *leading_dims, atoms, must_be[3] = x_0.shape
    batch = prod(leading_dims)
    x_0 = x_0.reshape(batch, atoms, 3)
    x_1_noised = x_0 + self.sigma_t(torch.zeros(1, device=x_0.device))[:, None, None]*self.randgen.randn(1, x_0.shape[:-1])
    ...
    for i in range(steps):
      ...
      _, x_1_pred = self.dn(torch.ones_like(t), t, x_0, x_1_noised, self.box, metadata)
      pred_noise = x_1_noised - x_1_pred
      x_1_noised -= ((dsigma/sigma_t)**2)*pred_noise
      epsilon = self.randgen.randn(1, x_0.shape[:-1])
      x_1_noised += (dsigma*sigma_tdec/sigma_t)*epsilon
    return x_1_noised.reshape(*leading_dims, atoms, 3)
...
class DiffusionDenoiserTrainer:
  ...
  def step(self, i, trajs):
    loss, denoise_loss, reg_loss = self.model.train_step(trajs)
    print(f"{i}\t ℒ = {loss:05.6f} \t ℒᴰ = {denoise_loss:05.6f} \t ℒᴿ = {reg_loss:05.6f}")
    self.board.scalar("loss", i, loss)
    self.board.scalar("denoise_loss", i, denoise_loss)
    self.board.scalar("reg_loss", i, reg_loss)
