...
class GlobalLin(nn.Module):
  """ Linear layer that treats different positions on the chain differently. """
  def __init__(self, config:Config):
    super().__init__()
    adim = config["adim"]
    subdim = adim // 4
    nodes = config.predictor.shape[0]
    self.lin_in = nn.Linear(adim, subdim, bias=False)
    self.lin_1 = nn.Linear(nodes*subdim, 8*subdim)
    self.lin_2 = nn.Linear(8*subdim, nodes*subdim)
    self.lin_out = nn.Linear(subdim, adim, bias=False)
  def forward(self, x):
    x = self.lin_in(x)
    batch, nodes, subdim = x.shape
    y = self.lin_2(self.lin_1(x.reshape(batch, nodes*subdim))).reshape(batch, nodes, subdim)
    return self.lin_out(y)


class GlobalVecLin(nn.Module):
  """ Linear layer that treats different positions on the chain differently. """
  def __init__(self, config:Config):
    super().__init__()
    vdim = config["vdim"]
    subdim = vdim // 4
    nodes = config.predictor.shape[0]
    self.lin_in = VecLinear(vdim, subdim)
    self.lin_1 = VecLinear(nodes*subdim, 8*subdim)
    self.lin_2 = VecLinear(8*subdim, nodes*subdim)
    self.lin_out = VecLinear(subdim, vdim)
  def forward(self, x):
    x = self.lin_in(x)
    batch, nodes, subdim, must_be[3] = x.shape
    y = self.lin_2(self.lin_1(x.reshape(batch, nodes*subdim, 3))).reshape(batch, nodes, subdim, 3)
    return self.lin_out(y)
...
class LocalResidual(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.global_a = GlobalLin(config)
    self.global_v = GlobalVecLin(config)
  ...
  def forward(self, x_a, x_v):
    ...
    g_a, g_v = self.global_a(x_a), self.global_v(x_v)
    z_a, z_v = self.gnorm_a(y_a + p_a + g_a), self.gnorm_v(y_v + p_v + g_v)
...
class Block(nn.Module):
  def __init__(self, config):
    ...
    self.conv_1_a = ScalConv1d(adim, 5)
    self.conv_1_v = VecConv1d(vdim, 5)
    ...
    self.prox_attn = ProximityFlashAttention(config["r0_list"], adim, vdim, 16)
    self.gnorm_a = ScalGroupNorm(adim, agroups)
    self.gnorm_v = VecGroupNorm(vdim, vgroups)
  ...
  def forward(self, tup):
    ...
    y_a = self.conv_1_a(self.conv_0_a(x_a)) + emb_a
    y_v = self.conv_1_v(self.conv_0_v(x_v)) + emb_v
