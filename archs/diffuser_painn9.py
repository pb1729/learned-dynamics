...
class SE3ITO(nn.Module):
  def __init__(self, r0:float, n:int, blks_0:int=2, blks_1:int=5):
    super().__init__()
    self.r0 = r0
    self.atom_embeddings = ResidueAtomEmbed(n)
  ...
  def forward(self, t:torch.Tensor, x0:torch.Tensor, x1:torch.Tensor, box, metadata:OpenMMMetadata):
    ...
    x_a = self.atom_embeddings(metadata)[None].expand(x0.shape[0], -1, -1)
...
class DiffusionDenoiser:
  ...
  def __init__(self, config):
    ...
    self.config = config
    self.dn = SE3ITO(config["r0"], config["n"]).to(config.device)
  ...
  def _diffuser_step(self, x, metadata):
    ...
    self.optim.step()
    return loss.item()
  def generate(self, x_0, metadata, steps=16):
