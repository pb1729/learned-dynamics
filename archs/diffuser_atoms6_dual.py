...
class SymmetricDenoiser(nn.Module):
  ...
  def forward(self, t, pos_0, pos_1, box, metadata):
    pos_0_dn_0_to_1, pos_1_dn_0_to_1 = self.directed_dn(t, pos_0, pos_1, box, metadata)
    pos_1_dn_1_to_0, pos_0_dn_1_to_0 = self.directed_dn(t, pos_1, pos_0, box, metadata)
...
class DiffusionDenoiser:
  ...
  def _diffuser_step(self, x, metadata):
    ...
    noise_0 = sigma*epsilon_0
    noise_1 = sigma*epsilon_1
    x_0_noised = x_0 + noise_0
    x_1_noised = x_1 + noise_1
    noise_0_pred, noise_1_pred = self.dn(t, x_0_noised, x_1_noised, self.box, metadata)
    loss = (((noise_0_pred - noise_0)/(0.4 + sigma))**2).mean() + (((noise_1_pred - noise_1)/(0.4 + sigma))**2).mean()
    #loss = (((noise_0_pred - noise_0)**2 + (noise_1_pred - noise_1)**2) / (0.2 + sigma**2)).mean()
  ...
  def generate(self, x_0:torch.Tensor, metadata, steps=12):
    *leading_dims, atoms, must_be[3] = x_0.shape
    ...
    for i in range(steps):
      ...
      x_0_noised = x_0 + sigma_t*self.randgen.randn(1, x_0.shape[:-1])
      _, noise_1_pred = self.dn(t, x_0_noised, x_1_noised, self.box, metadata)
