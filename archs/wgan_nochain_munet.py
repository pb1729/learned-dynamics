...
class Residual(nn.Module):
  ...
  def forward(self, x):
    prod = self.lin_o(self.lin_l(x)*self.lin_r(x))
    prod = prod/torch.sqrt(1. + prod**2) # prevent from getting too big
    return x + self.layers(x) + prod
...
class Generator(nn.Module):
  def __init__(self, config):
    ...
    self.layers2 = nn.Sequential(
      ResidualLin(ngf),
      ResidualLin(ngf))
    self.layers_mu_pred = nn.Sequential(
      nn.Linear(dim, ngf),
      Residual(ngf),
      Residual(ngf),
      ResidualLin(ngf),
  ...
  def forward(self, noise, x0):
    ...
    base = self.layers_mu_pred(x0)
    return base, base + noise[:, 0::15] + self.readout(self.layers2(
        self.input_embed2(inp) + self.layers1(
            self.input_embed1(inp)
        )
    ))
...
class GAN:
  ...
  def disc_step(self, r_data, cond):
    ...
    base, g_data = self.gen(self.get_latents(cond.shape[0]), cond) # note that we noise from 0 rather than starting from cond...
    ...
    self.optim_d.step()
    return loss.item()
  def gen_step(self, r_data, cond):
    self.optim_g.zero_grad()
    base, g_data = self.gen(self.get_latents(cond.shape[0]), cond)
    y_g = self.disc(g_data, cond)
    mse_loss = 0.5*((base - r_data)**2).mean()
    loss = y_g.mean() + mse_loss
    loss.backward()
    self.optim_g.step()
    return loss.item()
  ...
  def predict(self, cond):
    batch = cond.shape[0]
    return self.gen(self.get_latents(batch), cond)[1]
  ...
  def record_frame(self, i, hist_datas=1000):
    with torch.no_grad():
      ...
      base, x1_hat = self.gen(self.get_latents(hist_datas), x0.expand(hist_datas, 1))
      x1_hat = x1_hat.detach().cpu()
      ax.hist(x1[:, 0], bins=20, alpha=0.4)
      ax.hist(x1_hat[:, 0], bins=20, alpha=0.4)
      ax.scatter(base[0:1].detach().cpu(), [0.])
