...
class Block(nn.Module):
  def __init__(self, config):
    ...
    #self.node_embed = GraphEmbedLayer(self.r_cut)
    #self.lin_g = VecLinear(vdim, 1)
  ...
  def forward(self, tup):
    ...
    #pos_g = pos_0 + self.lin_g(x_v)[:, :, 0]
    #graph = Graph.radius_graph(self.r_cut, self.box, pos_g, neighbours_max=64)
    #emb_a, emb_v = self.node_embed(graph, pos_0)
    y_a = x_a #+ self.emb_a(emb_a)
    y_v = x_v + self.lin_emb_v((pos_1 - pos_0)[:, :, None]) #+ self.emb_v(emb_v)
    z_a, z_v = self.local_res(y_a, y_v)
    probes_q = self.probe_pts_q(z_v, pos_0)
    probes_k = self.probe_pts_k(z_v, pos_0)
    dz_a, dz_v = self.prox_attn(z_a, z_v, probes_k, probes_q, self.box)
