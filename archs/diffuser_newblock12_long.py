...
class PosEmbed(nn.Module):
  ...
  def __init__(self, poscount, dim_a, dim_v):
    super().__init__()
    self.poscount = poscount
    self.lin_a = TensLinear(0, poscount**2, dim_a)
    self.lin_v = TensLinear(1, poscount**2, dim_v)
  def forward(self, poses):
    """ poses: (poscount, batch, nodes, 3) """
    must_be[self.poscount], batch, nodes, must_be[3] = poses.shape
    delta = (poses[:, None] - poses[None, :]).reshape(self.poscount**2, batch, nodes, 3)
    delta = delta.permute(1, 2, 0, 3) # (batch, nodes, poscount**2, 3)
    return self.lin_a(0.01*(delta**2).sum(-1)), self.lin_v(delta)
...
class NeighbourDispEmbed(nn.Module):
  ...
  def __init__(self, poscount, dim_v):
    super().__init__()
    self.lin_l = TensLinear(1, poscount, dim_v)
    self.lin_r = TensLinear(1, poscount, dim_v)
  def forward(self, poses):
    """ poses: (poscount, batch, nodes, 3)
        ans: (batch, nodes, dim_v, 3) """
    poses = poses.permute(1, 2, 0, 3) # (batch, nodes, poscount, 3)
    delta = poses[:, 1:] - poses[:, :-1] # (batch, nodes - 1, poscount, 3)
    delta_l = F.pad(delta, (0,0,  0,0,  0,1)) # (batch, nodes, poscount, 3)
    delta_r = F.pad(delta, (0,0,  0,0,  1,0)) # (batch, nodes, poscount, 3)
    return 0.1*(self.lin_l(delta_l) + self.lin_r(delta_r))


class ProbePoints(nn.Module):
  """ Create a position from a mix of existing positions, plus some vector activations. """
  def __init__(self, poscount, dim_v):
    super().__init__()
    self.poscount = poscount
    self.dim_v = dim_v
    self.W_p = nn.Parameter(torch.zeros(poscount))
    self.lin_actv = TensLinear(1, dim_v, poscount)
  def self_init(self):
    with torch.no_grad():
      self.W_p[-1] += 1.
      self.lin_actv.W.zero_()
  def forward(self, poses, x_v):
    must_be[self.poscount], batch, nodes, must_be[3] = poses.shape
    must_be[batch], must_be[nodes], must_be[self.dim_v], must_be[3] = x_v.shape
    poses = poses + self.lin_actv(x_v).permute(2, 0, 1, 3)
    W_p_normal = self.W_p + (1. - self.W_p.sum())/self.poscount
    return (poses*W_p_normal[:, None, None, None]).sum(0)
...
class Block(nn.Module):
  ...
  def __init__(self, config:Config):
    ...
    self.pos_embed = PosEmbed(config["framelen"] + 1, dim_a, dim_v)
    self.ndisp_embed = NeighbourDispEmbed(config["framelen"] + 1, dim_v)
    self.res_embed = ResidueEmbed(dim_a)
    self.sublock_0 = SubBlock(config)
    self.probept_0 = ProbePoints(config["framelen"], dim_v)
    ...
    self.lin_push_0_pos_0s = TensLinear(1, dim_v, config["framelen"])
    self.lin_push_0_pos_1 = TensLinear(1, dim_v, 1)
    self.lin_push_1_pos_0s = TensLinear(1, dim_v, config["framelen"])
  ...
  def self_init(self):
    with torch.no_grad():
      self.lin_push_0_pos_0s.W.zero_()
      self.lin_push_0_pos_1.W.zero_()
      self.lin_push_1_pos_0s.W.zero_()
      self.lin_push_1_pos_1.W.zero_()
  def forward(self, tup):
    """ t: (batch)
        pos_0s: (framelen, batch, nodes, 3)
        pos_1: (batch, nodes, 3)
        x_a: (batch, nodes, dim_a)
        x_v: (batch, nodes, dim_v, 3)
        x_d: (batch, nodes, dim_d, 3, 3) """
    t, pos_0s, pos_1, x_a, x_v, x_d, box, metadata = tup
    # residual connections
    res_a, res_v, res_d = x_a, x_v, x_d
    # embed time
    x_a = x_a + self.embed_t(t)[:, None]
    # simple embed positions
    x_a = x_a + self.arc_embed(pos_1.shape[0], pos_1.shape[1], pos_1.device)
    pos_all = torch.cat([pos_0s, pos_1[None]], dim=0)
    x_v = x_v + self.ndisp_embed(pos_all)
    Δx_a, Δx_v = self.pos_embed(pos_all)
    ...
    pos_0s = pos_0s + self.lin_push_0_pos_0s(x_v).permute(2,0,1,3)
    pos_1 = pos_1 + self.lin_push_0_pos_1(x_v).squeeze(2)
    # make graphs
    pos_probe = self.probept_0(pos_0s, x_v)
    graph_0, r_ij_0 = graph_setup(self.r0, box, pos_probe)
    ...
    # push positions by some vectors
    pos_0s = pos_0s + self.lin_push_1_pos_0s(x_v).permute(2,0,1,3)
    pos_1 = pos_1 + self.lin_push_1_pos_1(x_v).squeeze(2)
    # linear map to add to residuals
    x_a, x_v, x_d = self.lin_out_a(x_a), self.lin_out_v(x_v), self.lin_out_d(x_d)
    return t, pos_0s, pos_1, res_a + x_a, res_v + x_v, res_d + x_d, box, metadata
...
class Denoiser(nn.Module):
  def __init__(self, config):
    ...
    self.framelen = config["framelen"]
    self.r0:float = config["r_cut"] # cutoff radius
    self.encode_0 = ResiduesEncode(self.dim_v, nlin=2)
    self.actv_encode_0 = TensLinear(1, self.framelen*self.dim_v, self.dim_v)
  ...
  def forward(self, t, pos_0s, pos_1, box, metadata):
    must_be[self.framelen], batch, atoms, must_be[3] = pos_0s.shape
    must_be[batch], must_be[atoms], must_be[3] = pos_1.shape
    device = pos_0s.device
    pos_0s, x_v_0 = self.encode_0(pos_0s.reshape(self.framelen*batch, atoms, 3), metadata)
    must_be[self.framelen*batch], nodes, must_be[3] = pos_0s.shape
    must_be[self.framelen*batch], nodes, must_be[self.dim_v], must_be[3] = x_v_0.shape
    pos_0s = pos_0s.reshape(self.framelen, batch, nodes, 3)
    x_v_0 = x_v_0.reshape(self.framelen, batch, nodes, self.dim_v, 3)
    x_v_0 = self.actv_encode_0(x_v_0.permute(1, 2, 0, 3, 4).reshape(batch, nodes, self.framelen*self.dim_v, 3))
    ...
    tup = (t, pos_0s, pos_1, x_a, x_v, x_d, box, metadata)
    tup = self.blocks(tup)
