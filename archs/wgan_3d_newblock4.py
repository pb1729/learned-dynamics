...
class Discriminator(nn.Module):
  def __init__(self, config, randgen):
    ...
    self.taxicab_scale = nn.Parameter(torch.tensor(0.001)) # scale down for stability!
  def forward(self, pos_0, pos_1, box):
    ...
    y = self.taxicab_scale*taxicab(pos_1, pos_2)[:, None] # (batch, 1)
