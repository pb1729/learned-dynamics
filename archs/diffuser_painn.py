...
class Update(nn.Module):
  ...
  def forward(self, x_a, x_v):
    ...
    inner_prod = inner_prod*y[..., self.n:2*self.n]
...
class SE3ITO(nn.Module):
  ...
  def forward(self, t:torch.Tensor, x0:torch.Tensor, x1:torch.Tensor, box, metadata:OpenMMMetadata):
    ...
    x_v = torch.zeros(*x_a.shape, 3, device=x_a.device) # Only legal equivariant input: r_ij will make this non-zero in subsequent layers
    # TODO: note: we could have used stuff like x0 minus noised x1 above. The arch in the paper incorporates that info differently, tho.
...
class DiffusionDenoiser:
  ...
  def _diffuser_step(self, x, metadata):
    ...
    noise_pred = self.dn(t, x_0, x_1_noised, self.box, metadata)
    loss = (((noise_pred - noise)/(0.4 + sigma))**2).mean()
  ...
  def generate(self, x_0, metadata, steps=12):
    *leading_dims, atoms, must_be[3] = x_0.shape
    ...
    for i in range(steps):
      ...
      pred_noise = self.dn(t, x_0, ans, self.box, metadata)
